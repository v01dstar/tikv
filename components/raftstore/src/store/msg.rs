...
use engine_traits::{CompactedEvent, KvEngine, RangeStats, Snapshot};
...
use kvproto::{
    ...
    metapb::{self, Region, RegionEpoch, Store},
    ...
    fn fmt(&self, fmt: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            StoreMsg::CollectWholeRangeMVCCStats { mvcc_stats } => {
                write!(fmt, "CollectWholeRangeMVCCStats {:?}", mvcc_stats)
            }
            StoreMsg::FlushEvent(event) => write!(fmt, "FlushEvent {:?}", event.cf()),
            #[cfg(any(test, feature = "testexport"))]
...
impl<EK: KvEngine> StoreMsg<EK> {
    pub fn discriminant(&self) -> usize {
        match self {
            ...
            StoreMsg::CollectWholeRangeMVCCStats { .. } => 12,
            StoreMsg::FlushEvent(_) => 13,
            #[cfg(any(test, feature = "testexport"))]
